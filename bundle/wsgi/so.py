...
class SOE(service_orchestrator.Execution):
    ...
    def update(self, old, new, extras):
        if self.stack_id is not None:
            ...
            try:
                mme = new.attributes['mcn.endpoint.mme-pgwc-sgwc']
            except KeyError:
                LOG.error('The update method to the service instance was called but a required parameter was not found.')
                LOG.error('mcn.endpoint.mme-pgwc-sgwc must be supplied as an X-OCCI-Attribute header.')
                LOG.error('Doing nothing...')
                raise RuntimeError('mcn.endpoint.mme-pgwc-sgwc must be supplied as an X-OCCI-Attribute header.')
